...
def get_day():
    ...
    d1 = days[day_of_week]
    d2 = days[(day_of_week + 1) % len(days)]  # Wrap around the index using modulo
    d3 = days[(day_of_week + 2) % len(days)]  # Wrap around the index using modulo
    d4 = days[(day_of_week + 3) % len(days)]  # Wrap around the index using modulo
...
def check_c():
    ...
    if user_input.lower() == 'mumbai':
        ...
        return x+4
    elif user_input.lower() == 'bhubneshwar':
        return x+5
...
# label3 = Label(frame,wraplength=200 ,text="please drink water and stay hidrated take rest",width=25,height=6, font=('Helvetica', 11), fg="white", bg="#272928")
